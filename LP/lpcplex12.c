...
/****************************************************************************/
/*                                                                          */
/*              Interface Routines to The CPLEX 12 LP Solver                */
/*                                                                          */
/*  NOTE: Use this code in place of lp_none.c to access the Cplex 12        */
...
static int
    primalopt (CClp *lp, int *infeasible),
    dualopt (CClp *lp, int *infeasible),
    baropt (CClp *lp),
...
/* Extra function calls for mipper.c  Bico - 110313 */
...
int CClp_opt (CClp *lp, int method, int *infeasible)
{
    int rval = 0;

    if (infeasible) *infeasible = 0;
    ...
    switch (method) {
        case CClp_METHOD_PRIMAL:
            rval = primalopt (lp, infeasible);
            break;
        case CClp_METHOD_DUAL:
            rval = dualopt (lp, infeasible);
            break;
...
int cplex_write_prob (CClp *lp)
{
    ...
    strncpy (lpbuf, "LP", sizeof ("LP"));

    rval = CPXwriteprob (lp->cplex_env, lp->cplex_lp, "my.lp", lpbuf);
    CCcheck_rval (rval, "CPXwriteprob failed");
...
static int primalopt (CClp *lp, int *infeasible)
{
    int rval;
    int solstat;
...
static int dualopt (CClp *lp, int *infeasible)
{
    int rval;
    int solstat;
...
static int baropt (CClp *lp)
{
    ...
    /* Setting THREADS to default (was set to 1 in version 12.2) */

    rval = CPXsetintparam (lp->cplex_env, CPX_PARAM_THREADS, 0); 
    CCcheck_rval (rval, "CPXsetintparam failed -- CPX_PARAM_THREADS");

    rval = CPXsetintparam (lp->cplex_env, CPX_PARAM_SCRIND, 1);
    CCcheck_rval (rval, "CPXsetintparam failed -- CPX_PARAM_SCRIND");
...
/*
        ...
        return dualopt (lp);
*/
        printf ("CPXbaropt returned non-optimal solution\n");
    }
...
static int set_parameters (CPXENVptr cplex_env, CClp_parameters *params)
{
    ...
    /* BUG IN CPLEX 12.2, need to set CPX_PARAM_TREADS to 1 */
...
void CClp_changeobjectivesense (CClp *lp, int sense)
{
    if (sense != 0) sense = CPX_MAX;
    else            sense = CPX_MIN;

    CPXchgobjsen (lp->cplex_env, lp->cplex_lp, sense);
...
int CClp_integervariables (CClp *lp)
{
    ...
    ncols = CPXgetnumcols (lp->cplex_env, lp->cplex_lp);
    ind = CC_SAFE_MALLOC (ncols, int);
    CCcheck_NULL (ind, "out of memory for ind");
    ctype = CC_SAFE_MALLOC (ncols, char);
    CCcheck_NULL (ctype, "out of memory for ctype");
    ...
    rval = CPXchgctype (lp->cplex_env, lp->cplex_lp, ncols, ind, ctype);
    CCcheck_rval (rval, "CPXchgctype failed");
...
int CClp_mipopt (CClp *lp)
{
    ...
    printf ("MIP Time Limit: %.2f seconds\n", tlim); fflush (stdout);

    rval = CPXsetdblparam (lp->cplex_env, CPX_PARAM_TILIM, tlim);
    CCcheck_rval (rval, "CPXsetdblparam TILIM failed");

    rval = CPXsetdblparam (lp->cplex_env, CPX_PARAM_CUTLO, -4.01);
    CCcheck_rval (rval, "CPXsetdblparam TILIM failed");

    rval = CPXsetintparam (lp->cplex_env, CPX_PARAM_MIPEMPHASIS,
                                          CPX_MIPEMPHASIS_FEASIBILITY);
    CCcheck_rval (rval, "CPXsetdblparam TILIM failed");
    
    rval = CPXmipopt (lp->cplex_env, lp->cplex_lp);
    if (rval) {
        printf ("CPXmipopt returned value %d\n", rval);  fflush (stdout);
    }

    solstat = CPXgetstat (lp->cplex_env, lp->cplex_lp);
    switch (solstat) {
        case CPXMIP_OPTIMAL:
            printf ("MIP optimal solution has been found\n");
            break;
        case CPXMIP_TIME_LIM_FEAS:
            printf ("MIP hit time limit with an integer solution\n");
            break;
        case CPXMIP_TIME_LIM_INFEAS:
            printf ("MIP hit time limit without an integer solution\n");
            break;
        case CPXMIP_ABORT_FEAS:
            printf ("MIP aborted with an integer solution\n");
            break;
        case CPXMIP_ABORT_INFEAS:
            printf ("MIP aborted without an integer solution\n");
            break;
        default:
            printf ("MIP Solution Status: %d\n", solstat);
            break;
    }
    fflush (stdout);
 
    rval = CPXsetdblparam (lp->cplex_env, CPX_PARAM_TILIM, 1.0E75);
    CCcheck_rval (rval, "CPXsetdblparam TILIM failed");

    rval = CPXsetdblparam (lp->cplex_env, CPX_PARAM_CUTLO, -1.0E75);
    CCcheck_rval (rval, "CPXsetdblparam TILIM failed");
