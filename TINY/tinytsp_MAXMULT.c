...
#define MAX_MULT 65536.0  /* Used to skip nasty dual LPs */
...
static int tiny_checkdual (tiny_lp *lp, int *cutoff)
{
    ...
    for (i = 0; i < ncount; i++) {
        if (lp->pi_double[i] > MAX_MULT || lp->pi_double[i] < -MAX_MULT) {
            printf ("Stopping tinytsp due to large LP multiplier\n");
            rval = 1;
        }
    }
    for (i = 0; i < lp->ncuts; i++) {
        if (lp->pi_double[ncount+i] > MAX_MULT) {
            printf ("Stopping tinytsp due to large LP multiplier\n");
            rval = 1;
        }
    }
