...
static int
    ...
    pricing_duals (CCtsp_lp *lp, double *node_pi, double *node_piest,
           ...
           double *node_pi_dom, double *domino_pi),
    ...
    price_list_new (CCtsp_lp *lp, int ecount, CCtsp_predge *elist,
        ...
        double *node_pi_dom, double *domino_pi, int phase1),
...
static double non_time = 1.0, nz_time = 1.0, dp_time1 = 1.0; /* YYY */
static double dp_time2 = 1.0, dp_time3 = 1.0;
static int counter_dom = 0;
        ...
        int phase1, int *feasible, int silent, CCrandstate *rstate)
{
    ...
    CCtsp_predge *new_inlist = (CCtsp_predge *) NULL;
    CCtsp_predge *new_prlist = (CCtsp_predge *) NULL;

    azeit = CCutil_zeit();
    non_time = nz_time = dp_time1 = dp_time2 = dp_time3 = 0.0;
    counter_dom = 0;
    ...
    CC_MALLOC (new_prlist, PRICE_POOL + ngen, CCtsp_predge);
    ...
    *pnadded = 0;
    ...
    while (!finished) {
        if (phase1) {
            ...
            printf ("Generate Time: %f (%f)\n", gen_time, 
                                       gen_time / (CCutil_zeit() - azeit));
            printf ("Price Time: %f (%f) (non portion %f: %f %f %f %f -> %d)\n", price_time, 
                                       price_time / (CCutil_zeit() - azeit),
                                       non_time / price_time, 
                                       nz_time / non_time,
                dp_time1 / non_time, dp_time2 / non_time, dp_time3 / non_time,
                counter_dom);
            fflush (stdout);
        }
        ...
        if (incount) {
            CC_MALLOC (new_inlist, incount, CCtsp_predge);
            for (i = 0; i < incount; i++) {
                new_inlist[i].ends[0] = inlist[i].ends[0];
                new_inlist[i].ends[1] = inlist[i].ends[1];
                new_inlist[i].len = inlist[i].len;
            }
        }

        rval = price_list_new (lp, incount, new_inlist, node_pi, cut_pi,
                               clique_pi, node_pi_dom, domino_pi, phase1);
        CCcheck_rval (rval, "price_list_new failed");

        price_time += CCutil_zeit() - szeit;

        for (i = 0; i < incount; i++) {
            if (inlist[i].rc > new_inlist[i].rc + 0.00001) {
                printf ("DIFF A: %f\n", inlist[i].rc - new_inlist[i].rc);
                fflush (stdout);
            } else if (new_inlist[i].rc > inlist[i].rc + 0.00001) {
                printf ("DIFF B: %f\n", new_inlist[i].rc - inlist[i].rc);
                fflush (stdout);
            }
        }

        CC_IFFREE (new_inlist, CCtsp_predge);
        ...
        while ((!finished && prcount >= PRICE_POOL) ||
            ...
            *pnadded += n;
            ...
            if (prcount) {
                CC_MALLOC (new_prlist, prcount, CCtsp_predge);
                for (i = 0; i < prcount; i++) {
                    new_prlist[i].ends[0] = prlist[i].ends[0];
                    new_prlist[i].ends[1] = prlist[i].ends[1];
                    new_prlist[i].len = prlist[i].len;
                }
            }

            rval = price_list_new (lp, prcount, new_prlist, node_pi, cut_pi,
                                   clique_pi, node_pi_dom, domino_pi, phase1);
            CCcheck_rval (rval, "price_list_new failed");

            for (i = 0; i < prcount; i++) {
                if (prlist[i].rc > new_prlist[i].rc + 0.00001) {
                    printf ("DIFF C: %f\n", prlist[i].rc - new_prlist[i].rc);
                    fflush (stdout);
                } else if (new_prlist[i].rc > prlist[i].rc + 0.00001) {
                    printf ("DIFF D: %f\n", new_prlist[i].rc - prlist[i].rc);
                    fflush (stdout);
                }
            }

            CC_IFFREE (new_prlist, CCtsp_predge);
        ...
        double *node_pi, double *cut_pi, double *clique_pi, int phase1)
{
    ...
    printf ("Non-hypergraph cuts: %d/%d\n", noncnt, lp->cuts.cutcount);
    fflush (stdout);
...
#if 0   /* YYY - this removes all DP cuts */
