...
int CCtsp_register_clique (CCtsp_lpcuts *cuts, CCtsp_lpclique *c)
{
    ...
    if (cuts->cliquefree != -1) {
        ...
        if (cuts->cliqueend >= cuts->cliquespace) {
            void *tmp_ptr = (void *) cuts->cliques;
            if (CCutil_reallocrus_scale (&tmp_ptr, &cuts->cliquespace,
                    cuts->cliqueend + 1, 1.3, sizeof (CCtsp_lpclique))) {
                CC_FREE (new, CCtsp_segment);
                return -1;
            }
            cuts->cliques = (CCtsp_lpclique *) tmp_ptr;
        }
        y = cuts->cliqueend++;
    }
...
int CCtsp_register_domino (CCtsp_lpcuts *cuts, CCtsp_lpdomino *c)
{
    ...
    if (cuts->dominofree != -1) {
        ...
        if (cuts->dominoend >= cuts->dominospace) {
            void *tmp_ptr = (void *) cuts->dominos;
            if (CCutil_reallocrus_scale (&tmp_ptr, &cuts->dominospace,
                    cuts->dominoend + 1, 1.3, sizeof (CCtsp_lpdomino))) {
                CC_FREE (new[0], CCtsp_segment);
                CC_FREE (new[1], CCtsp_segment);
                return -1;
            }
            cuts->dominos = (CCtsp_lpdomino *) tmp_ptr;
        }
        y = cuts->dominoend++;
    }
