...
/****************************************************************************/
/*                                                                          */
/*                    BOSS FOR PARALLEL CUTTING ROUTINES                    */
/*                                                                          */
/*                                TSP CODE                                  */
/*                                                                          */
/*                                                                          */
...
{
    (void) u_data;  /* to avoid unused parameter warning */
...
{
    int i, id, ncount, nremain, scount, nfail = 0, iopen = 0, rval = 0;
    CCgenhash idmap;
    int genhash_created = 0;
    CC_SPORT *lport = (CC_SPORT *) NULL;
    CC_SFILE *s;
    double rtime, rbound, ibound = 0.0, cumtime = 0.0, cumbound = 0.0;
    ...
    for (i = 0; i < scount; i++) {
        if (slist[i].bound <= 0.0) {
             slist[i].status = SUB_STAT_OPEN;
             iopen++;
        } else {
             slist[i].status = SUB_STAT_DONE;
             ibound += slist[i].bound;
        }
    }

    if (iopen == 0) {
        printf ("No work.  Bound is %.2f\n", ibound);
        goto CLEANUP;
    } else if (ibound > 0.0) {
    ...
    rval = CCutil_genhash_init (&idmap, scount, idcmp, idhash, NULL, 0.8, 0.4);
    CCcheck_rval (rval, "CCutil_genhash_init failed");
    genhash_created = 1;

    for (i=0; i < scount; i++) {
        rval = CCutil_genhash_insert (&idmap, (void *) (long) slist[i].id,
                                      (void *) &slist[i]);
    ...
    while (nremain) {
        ...
        p = (CCsubdiv *) CCutil_genhash_lookup (&idmap, (void *) (long) id);
        if (!p) {
