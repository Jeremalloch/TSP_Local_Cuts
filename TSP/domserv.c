...
static unsigned short cutport = CCtsp_DOMCUT_PORT;
...
static int 
    serve_request (CC_SFILE *f, int ncount, CCtsp_lpcuts *pool, int *status,
        int *starter, CCrandstate *rstate),
    find_cuts (CC_SFILE *f, int ncount, CCtsp_lpcuts *pool,
        int *starter, CCrandstate *rstate),
...
int main (int ac, char **av)
{
    ...
    do {
        ...
        rval = serve_request (f, nodecount, pool, &status, &starter, &rstate);
        if (rval) {
...
static int serve_request (CC_SFILE *f, int ncount, CCtsp_lpcuts *pool,
        int *status, int *starter, CCrandstate *rstate)
{
    char request;
    int rval;
    ...
    *status = STATUS_NORMAL;
    ...
    switch (request) {
      case CCtsp_POOL_GETCUTS:
        rval = find_cuts (f, ncount, pool, starter, rstate);
        if (rval) {
        ...
        int *starter, CCrandstate *rstate)
{
    ...
    rval = CCtsp_search_dominopool (pool, &cuts, &cutcount, &maxviol, ncount,
            ecount, elist, x, starter, 10000, 500, rstate);
    if (rval) {
...
static int receive_cuts (CC_SFILE *f, int ncount, CCtsp_lpcuts *pool)
{
    ...
    for (i=0; i<cutcount; i++) {
        ...
        CCcheck_rval (rval, "CCtsp_read_lpcut_in failed");
        /* Do not add non-dominos to pool */
        if (newc.dominocount > 0) {
...
static int parseargs (int ac, char **av)
{
    ...
    while ((c = CCutil_bix_getopt (ac, av, "n:p:P:s:", &boptind, &boptarg)) != EOF) switch (c) {
    case 'n':
