...
int main (int ac, char **av)
{
    ...
    if (tsplib == 0) {
        rval = CCtsp_solve_sparse (ncount, ecount, elist, elen, in_tour,
                  ...
                  probname, mytimebound, &hit_timebound,
                  (CCtsp_cutselect *) NULL, use_dfs, run_silently, &rstate);
        if (rval) {
        ...
        rval = CCtsp_solve_dat (ncount, &dat, in_tour, out_tour, mybnd,
                 ...
                 &hit_timebound, (CCtsp_cutselect *) NULL, use_dfs,
                 run_silently, &rstate);
        if (rval) {
...
static int parseargs (int ac, char **av)
{
    ...
    while ((c = CCutil_bix_getopt (ac, av, "do:Qs:t:Tu:z:", &boptind, &boptarg)) != EOF)
        switch (c) {
        case 'd':
            use_dfs = 1;
            break;
...
static void usage (char *f)
{
    ...
    fprintf (stderr, "   -d    use depth-first-search node selection\n");
