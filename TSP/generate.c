                    ...
                    if (supplyhead != -1) {
                        ...
                        if (current == 0) {
                            rval = xprice_reset (eg->xprice, eg->node_piest,
                                                 0, silent);
                            CCcheck_rval (rval, "xprice_reset failed");
                        }
                    }
                    xprice_node (eg->xprice, current, &supplycount, supply);
                    supplyhead = 0;
                }
        ...
        while (ngot < nwant) {
            if (supplyhead == supplycount || supplyhead == -1) {
                ...
                if (eg->kdtree) {
                    ...
                    CCcheck_rval (rval, "CCedgegen_node_k_nearest failed");
                } else {
                    rval = CCedgegen_junk_node_k_nearest (eg->dg,
                            (double *) NULL, current, eg->nneighbors,
                            eg->ncount, supply);
                    CCcheck_rval (rval, "CCedgegen_junk_node_k_nearest failed");
                }
                supplyhead = 0;
        ...
        int silent)
{
    ...
    CC_MALLOC (xp->pi, ncount, double);
    CC_MALLOC (xp->xminuspi_space, ncount+1, CCtsp_xnorm_pricer_val);
    CC_MALLOC (xp->invxminuspi, ncount, int);
    CC_MALLOC (xp->order, ncount, int);

    for (i = 0; i < ncount; i++) xp->order[i] = i;
    CCutil_double_perm_quicksort (xp->order, dat->x, ncount);

CLEANUP:
    if (rval) xprice_free (xp);
    return rval;
}
...
static int xprice_reset (CCtsp_xnorm_pricer *xp, double *pi, int currentnode,
        int silent)
{
    int rval = 0, i, norm, *xpiperm = (int *) NULL;
    ...
    for (i = 0; i < xp->ncount; i++) xp->pi[i] = pi[i];

    CC_MALLOC (xpi, count, double);
    CC_MALLOC (xpiperm, count, int);
...
static void xprice_node (CCtsp_xnorm_pricer *xp, int n, int *count, int *nlist)
{
    int k, norm;
    double kval, scale;
    ...
    *count = 0;
    ...
    if (xnp == xp->xminuspi) xp->xminuspi = xnp->next;
    else                     xnp->prev->next = xnp->next;
