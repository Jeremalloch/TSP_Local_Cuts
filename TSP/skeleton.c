...
int CCtsp_copy_skeleton (CCtsp_skeleton *old, CCtsp_skeleton *new)
{
    ...
    if (old->atomcount == 0) goto CLEANUP;

    CC_MALLOC (new->atoms, old->atomcount, int);
    for (i=0; i<old->atomcount; i++) { new->atoms[i] = old->atoms[i]; }
...
int CCtsp_construct_skeleton (CCtsp_lpcut_in *c, int nodecount)
{
    ...
    if (!CCtsp_hypergraph_type_in (c)) {
        printf ("Skeleton Yipes -- not a hypergraph cut\n"); fflush (stdout);
        rval = 1; goto CLEANUP;
    }

    CCtsp_init_skeleton (&c->skel);
    if (c->dominocount > 0) goto CLEANUP;
