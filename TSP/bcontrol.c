...
/*      char *poolfname, char *cutbossname, char *domcutbossname,           */
...
#define MAX_TENTATIVE_CHILDREN (256)
...
int CCtsp_knuth_probe (CCtsp_lp *lp, int *depth, int silent,
        CCrandstate *rstate);
        ...
        CCrandstate *rstate)
{
    ...
    CCcheck_NULL (rootbbnode, "failed to allocated root node");
...
int CCtsp_bfs_restart (char *probloc, char *restart_name, CCtsp_cutselect *sel,
        ...
        CCdatagroup *dat, int *ptour,
        CCtsp_lpcuts *pool, CCtsp_lpcuts *dominopool, int ncount,
        ...
        char *probloc, int silent, CCrandstate *rstate)
{
    ...
    info.dominopool = (CCtsp_lpcuts *) NULL;
    info.bblist = (tsp_bbnode *) NULL;
    ...
    rval = CCtsp_init_cutpool (&info.ncount, dompoolfname, &info.dominopool);
    CCcheck_rval (rval, "CCtsp_init_cutpool failed");
#endif


    if (cutbossname) {
        CCtsp_cutselect_remotepool (&cutselect, cutbossname);
        CCtsp_cutselect_remotepool (&tcutselect, cutbossname);
    }

    if (domcutbossname) {
        CCtsp_cutselect_remotedompool (&cutselect, domcutbossname);
        CCtsp_cutselect_remotedompool (&tcutselect, domcutbossname); 
    }
...
static int grunt_send_hello (tsp_bbinfo *info)
{
    ...
    for (i = 0; i <= 1; i++) {
        ...
        if (rval) goto CLEANUP;
        rval = CCutil_sread_int (f, &s->DP_cuts);
        if (rval) goto CLEANUP;
        rval = CCutil_sread_int (f, &s->TP_cuts);
        if (rval) goto CLEANUP;
        rval = CCutil_sread_int (f, &s->domino_safe_shrink);
        if (rval) goto CLEANUP;
    }
...
static int boss_receive_hello (CC_SFILE *f, tsp_bbinfo *info)
{
    ...
    for (i = 0; i <= 1; i++) {
        ...
        if (rval) return rval;
        rval = CCutil_swrite_int (f, s->DP_cuts);
        if (rval) return rval;
        rval = CCutil_swrite_int (f, s->TP_cuts);
        if (rval) return rval;
        rval = CCutil_swrite_int (f, s->domino_safe_shrink);
        if (rval) return rval;
    }
    ...
    switch (task->type) {
      ...
      case TASK_TENTATIVE_CUT:
        ...
        rval = CCtsp_bb_cutting (info->probloc, task->id, task->new_id, 
                ...
                info->pool, info->dominopool, info->tsel, &val, &prune,
                &foundtour, info->besttour, 0, info->silent, rstate);
        CCcheck_rval (rval, "CCtsp_bb_cutting failed");
        cputime = CCutil_zeit() - szeit;
        if (foundtour) {
            rval = report_tour (info);
            CCcheck_rval (rval, "report_tour failed");
        }
      ...
      case TASK_CUT:
        ...
        rval = CCtsp_bb_cutting (info->probloc, task->id, task->new_id, 
                info->ncount, info->dat, info->ptour, info->upbound,
                info->pool, info->dominopool, info->sel, &val, &prune,
                &foundtour,
        ...
        double val, double cputime, int standalone)
{
    ...
    if (standalone && info->hostname == (char *) NULL) {
        ...
        if (info->dominopool && info->dominopool->cutcount) {
            printf ("Writing Domino Pool: %d cuts\n",
                info->dominopool->cutcount);
            sprintf (buf, "%s.dompul", info->problabel);
            rval = CCtsp_write_cutpool (info->ncount, buf, info->dominopool);
            if (rval) {
                fprintf (stderr, "CCtsp_write_cutpool failed for dominos\n");
                return rval;
            }
        }
    }
...
static int update_tentative_bbnode (tsp_bbinfo *info, tsp_bbnode *b)
{
    ...
    if (i == b->numtentative) {
        ...
        double val;
        double bnd0, bnd1, upbnd = *(info->upbound); /* XXX Bico 170216 */

        printf ("Tentative Branching on BBnode %d\n", b->id);

        printf ("XX Upperbound = %.2f\n", *(info->upbound)); fflush (stdout);
        fflush (stdout);
        ...
        for (i = 0; i < b->numtentative; i++) {
            ...
            fflush (stdout);
            /* XX Bico 170216 */
            bnd0 = (t->child0->lowerbound > upbnd ? upbnd :
                                                    t->child0->lowerbound);
            bnd1 = (t->child1->lowerbound > upbnd ? upbnd :
                                                    t->child1->lowerbound);
            val = TSP_TENATIVE_BRANCH_VAL (bnd0, bnd1);
            /* End XXX Bico 170216 */
/*
...
*/
        ...
        if (info->save_proof == 0 /* || b->id != 0 Bico 060901 */) {
...
static int report_nobranch (tsp_bbinfo *info, int id, double cputime)
{
    ...
    *info->branchzeit += cputime;
    ...
    delete_bbnode (&info->bblist, bbnode);
    info->changed = 1;
    if (info->save_proof == 0 /* || id != 0 Bico 060901 */) {
        rval = CCtsp_prob_file_delete (info->probloc, id);
        if (rval) return rval;
    }
    return 0;
    ...
    *list = CC_SAFE_MALLOC (count, tsp_tnode);
    ...
    for (obtained = 0; obtained < count; obtained++) {
        ...
        rval = fscanf (f, "%d%d%lf%lf %d%d%lf%lf\n",
            ...
            /*                                            */
            /*  BICO 9 JANUARY 04 - Bug struck again at   */
            /*  the end of the Sweden run.  This does not */
            /*  hurt the run, but it is messy and should  */
            /*  be fixed.                                 */
...
static tsp_bbnode *select_bbnode (tsp_bbnode *firstbbnode, int verbose,
        int silent)
{
    int i, cutavail = 0, branchavail = 0, tcutavail = 0, active = 0;
    double bestbound  = CCtsp_LP_MAXDOUBLE;
    double lowerbound = CCtsp_LP_MAXDOUBLE;
    tsp_bbnode *b, *bestbbnode = (tsp_bbnode *) NULL;
    tsp_tnode *t;
...
int CCtsp_easy_dfs_brancher (CCtsp_lp *lp, CCtsp_cutselect *sel, int depth,
        ...
        int longedge_branching, int simple_branching, double *timebound,
        int *hit_timebound, int silent, CCrandstate *rstate)
{
    int rval = 0, infeasible = 0, ngot, prune, i, *cyc = (int *) NULL;
    double val, bnd, szeit, st, oldbound = lp->lowerbound;
    double tbound = CCtsp_LP_MAXDOUBLE, tzeit, *mytbound = (double *) NULL;
    CCtsp_branchobj *b = (CCtsp_branchobj *) NULL;
...
/*
    if (mytbound) {
        if (CCutil_zeit () - tzeit > *timebound) {
            fprintf (stderr, "Hit time limit in dfs branching\n");
...
*/
    ...
    if (!lp->full_edges_valid) { 
        fprintf (stderr, "need a valid full edge set\n");
        rval = 1; goto CLEANUP;
    }
    ...
    if (silent == 1) { printf ("%.2f -> ", bnd); fflush (stdout); }
    ...
    szeit = CCutil_zeit ();
    rval = CCtsp_cutting_loop (lp, sel, NULL, 0, silent, rstate, &infeasible);
    ...
    if (!silent) { printf ("Find branch object ...\n"); fflush (stdout); }

    szeit = CCutil_zeit ();
    if (simple_branching) {
        rval = CCtsp_find_fast_branch (lp, &ngot, &b, &val, &cyc, usecliques,
                                  longedge_branching, silent);
        CCcheck_rval (rval, "CCtsp_find_fast_branch failed");
    } else {
        rval = CCtsp_find_branch (lp, 1, &ngot, &b, &val, &cyc, usecliques, 
                                  longedge_branching, silent);
        CCcheck_rval (rval, "CCtsp_find_branch failed");
    }
    st = CCutil_zeit () - szeit;

    if (ngot == 0) {
        if (silent < 2) { printf ("TOUR FOUND: %.2f\n", val); fflush (stdout); }
        if (val < *upbound) {
            *upbound = val;
            lp->upperbound = val;
            for (i = 0; i < lp->graph.ncount; i++) besttour[i] = cyc[i];
        }
        CC_IFFREE (cyc, int);
        rval = CCtsp_verify_lp_prune (lp, &prune, silent);
        CCcheck_rval (rval, "CCtsp_verify_lp_prune failed");
    ...
    rval = CCtsp_execute_branch (lp, &b[0], silent, rstate, &infeasible);
    CCcheck_rval (rval, "CCtsp_execute_branch failed");
    if (infeasible) {
        if (silent < 2) {
            printf ("branched lp was infeasible\n"); fflush (stdout);
        }
        rval = CCtsp_verify_infeasible_lp (lp, &prune, silent);
        CCcheck_rval (rval, "CCtsp_verify_infeasible failed");
        if (prune) {
            if (silent < 2) {
        ...
        if (mytbound) tbound = (*timebound) - (CCutil_zeit () - tzeit);
    ...
    rval = CCtsp_execute_unbranch (lp, (CClp_warmstart *) NULL, silent, rstate,
                                   &infeasible);
    CCcheck_rval (rval, "CCtsp_execute_unbranch failed");
    if (infeasible) {
    ...
    if (infeasible) {
        ...
        if (mytbound) tbound = (*timebound) - (CCutil_zeit () - tzeit);
        rval = CCtsp_easy_dfs_brancher (lp, sel, depth + 1, upbound, bbcount,
                      usecliques, besttour, longedge_branching,
                      simple_branching, mytbound, hit_timebound,
                      silent, rstate);
        CCcheck_rval (rval, "CCtsp_easy_dfs_brancher failed");
    }
...
int CCtsp_knuth_probe (CCtsp_lp *lp, int *depth, int silent,
        CCrandstate *rstate)
{
    int rval = 0, infeasible = 0, ngot, *cyc = (int *) NULL;
    double val, bnd, szeit;
    CCtsp_branchobj *b = (CCtsp_branchobj *) NULL;
    CCtsp_cutselect sel;
    ...
    CCtsp_cutselect_dominos (&sel, 1, 1);
    ...
    printf ("Knuth Depth %d: ", *depth); fflush (stdout);
    ...
    rval = CCtsp_find_fast_branch (lp, &ngot, &b, &val, &cyc, 1, 1, silent);
    CCcheck_rval (rval, "CCtsp_find_fast_branch failed");

    if (ngot == 0) {
        printf ("PRUNE SEARCH: tour %.2f\n", val); fflush (stdout);
        goto CLEANUP;
    }

    if ((CCutil_lprand (rstate) % 2) == 0) {
        if (b[0].ends[0] != -1) {
            printf ("Branch: edge (%d, %d)->0\n", b[0].ends[0], b[0].ends[1]);
            fflush (stdout);
            b[0].rhs = 0;
        } else {
            printf ("Branch: set clique <= 2\n"); fflush (stdout);
            b[0].rhs = 2; b[0].sense = 'L';
        }
    } else {
        if (b[0].ends[0] != -1) {
            printf ("Branch: edge (%d, %d)->1\n", b[0].ends[0], b[0].ends[1]);
            fflush (stdout);
            b[0].rhs = 1;
        } else {
            printf ("Branch: set clique >= 4\n"); fflush (stdout);
            b[0].rhs   = 4; b[0].sense = 'G';
        }
    }
    ...
    if (infeasible) {
        ...
        (*depth)++;
        rval = CCtsp_knuth_probe (lp, depth, silent, rstate);
        CCcheck_rval (rval, "CCtsp_knuth_probe failed");
    }
