...
#define NAMESTRNUM(xa,xb,xc) (((xa)-'a') * 1296 + ((xb)-'a'+10) * 36 + \
        ...
        /* US50k HACK: Extend names to 5 digits for big search tree */
        n -= 1000;
        n -= 26*36*36 - 5;
        n -= 36*36*36*36;
...
int CCtsp_prob_putcuts (CCtsp_PROB_FILE *p, int ncount, CCtsp_lpcuts *cuts)
{
    ...
    if (rval) {
        fprintf (stderr, "begin_put failed\n");
        return rval;
    }
        ...
        int silent)
{
    ...
    if (!p) { rval = -1; goto CLEANUP; }

    rval = begin_get (p, p->offsets.cut, CCtsp_Pcuts, silent);
    CCcheck_rval (rval, "begin_get failed");
