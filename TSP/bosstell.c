...
static int telldomcutboss      = 0;
static int telldomboss         = 0;
static int tellverboss         = 0;
...
int main (int ac, char **av)
{
    ...
    if ((unsigned int) hostport == 0) {
        ...
        else if (telldomcutboss) hostport = (unsigned int) CCtsp_DOMCUT_PORT;
        else if (telldomboss) hostport = (unsigned int) CCtsp_DOMINO_PORT;
        else if (tellverboss) hostport = (unsigned int) CCtsp_VERIFY_PORT;
    ...
    if (tellcutboss || telldomcutboss) {
        ...
        if (send_exit) {
            rval = CCutil_swrite_string (f, "command");
            CCcheck_rval (rval, "CCutil_swrite_string failed");
            rval = CCutil_swrite_char (f, CCtsp_VERIFY_EXIT);
            CCcheck_rval (rval, "CCutil_swrite_char failed");
        } else if (savepool) {
            rval = CCutil_swrite_string (f, "command");
            CCcheck_rval (rval, "CCutil_swrite_string failed");
            rval = CCutil_swrite_char (f, CCtsp_VERIFY_SAVE);
            CCcheck_rval (rval, "CCutil_swrite_char failed");
        } else {
            fprintf (stderr, "Nothing to send to verboss\n");
            rval = 1; goto CLEANUP;
        }
    } else {
        if (deadnode >= 0) {
...
static int parseargs (int ac, char **av)
{
    ...
    while ((c = CCutil_bix_getopt (ac, av, "cd:DLp:sVx", &boptind, &boptarg)) != EOF) {
        switch (c) {
        case 'c':
            tellcutboss = 1;
            break;
...
static void usage (char *f)
{
    ...
    fprintf (stderr, "   or: %s -L [-p port] -x domcuthostname\n", f);
    fprintf (stderr, "   or: %s -V [-p port] -x verhostname\n", f);
    fprintf (stderr, "   -d n tells the boss that node n is dead\n");
    fprintf (stderr, "   -s   tells the cut(ver)boss to save the pool\n");
    fprintf (stderr, "   -x   tells the (cut,ver)boss to exit\n");
