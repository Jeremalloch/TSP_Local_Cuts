...
/*    -infeasible set to 1 if the LP becomes infeasible.                    */
/*                                                                          */
...
#define TSP_BRANCH_STRONG_ITERATIONS       (100)  /* 500 */
#define TSP_BRANCH_STRONG_EXTRA_ITERATIONS (500)  /* 5000 */
...
CLEANUP:
    CCutil_stop_timer (&lp->stats.strongbranch, silent);
    ...
    *ngot = 0;
    ...
    if (longedge_branching || usecliques) {
        if (longedge_branching) {
            ...
            if (rval) {
                fprintf (stderr, "find_longedge_cliques2 failed\n");
                goto CLEANUP;
            }
        } else {
    ...
    if (ccount) {
        ...
        if (infeasible) {
            fprintf (stderr, "Infeasible LP after branching test\n");
            rval = 1; goto CLEANUP;
        }
        ...
        CCtsp_lpclique **list, int silent)
{
    ...
    *ngot = 0;
    ...
    if (nwant <= 0) {
        fprintf (stderr, "find_longedge_cliques2 called with no nwant\n");
        rval = 1; goto CLEANUP;
    }
    ...
    if (len == (int *) NULL) {
        fprintf (stderr, "Out of memory in find_longedge_cliques2\n");
        rval = 1; goto CLEANUP;
    }
    for (i=0; i<xcount; i++) {
        ...
        int ccount, CCtsp_lpclique *clist, double *cval)
{
    ...
    *bobj = (CCtsp_branchobj *) NULL;
    ...
    printf ("Found %d items to choose in list\n", k); fflush (stdout);
    if (k == 0) {
        ...
        int **elist, double **eval, int silent)
{
    ...
    printf ("find_strongbranch_edges ...\n"); fflush (stdout);
    ...
    *elist = (int *) NULL;
    ...
    CCcheck_rval (rval, "find_candidate_edges failed");
    ...
    CCutil_stop_timer (&lp->stats.strongbranch_opt, silent);
        ...
        int **list, int silent)
{
    ...
    *list = (int *) NULL;
    ...
    if (nwant == TSP_BRANCH_STRONG_ALL_CHOICES) {
        ...
        CCcheck_rval (rval, "find_all_candidate_edges failed");
        goto CLEANUP;
    }
    ...
    CCcheck_rval (rval, "CClp_getgoodlist failed");
    if (!silent) {
        ...
        int *ngot, CCtsp_lpclique **bcliques, double **bval, int silent)
{
    ...
    *bcliques = (CCtsp_lpclique *) NULL;
    ...
    if (longedge_branching) {
        ...
        CCcheck_rval (rval, "find_longedge_cliques2 failed");
    } else {
        rval = find_candidate_cliques (lp, nwant * TSP_STRONG_CUT_CHOICES_MULT,
                                       &ccount, &cliques, 1, silent);
        CCcheck_rval (rval, "find_candidate_cliques failed");
    }
    if (!silent) {
        printf ("Found %d candidate cliques\n", ccount); fflush (stdout);
    }
    ...
    for (i = 0; i < ccount; i++) {
        ...
        CCcheck_rval (rval, "test_cut_branch failed");
        st = CCutil_stop_timer (&timer, 1);
        if (!silent) {
            printf ("SB CLIQUE %d:  %f  %f  (%.2f seconds)\n",
                       i, down, up, st);
    ...
    for (i = 0; i < lpcount; i++) {
        ...
        CCcheck_rval (rval, "test_cut_branch_failed");
        st = CCutil_stop_timer (&timer, 1);
        if (!silent) {
    ...
    CCcheck_rval (rval, "CClp_opt failed");
    if (infeasible) {
        fprintf (stderr, "Infeasible LP after 2nd branching test\n");
        rval = 1; goto CLEANUP;
    }
...
{
    int rval = 0, i, j, n0 = -1, n1 = -1;
    CCtsp_lpclique *c = (CCtsp_lpclique *) NULL;

    *infeasible = 0;

    if (!b) {
        fprintf (stderr, "CCtsp_execute_branch called without b\n");
        rval = 1; goto CLEANUP;
    }
    ...
    if (b->ends[0] != -1) {
        ...
        if (!silent) {
            ...
            if (b->sense == 'G') { printf ("to at least %d\n", b->rhs); }
            else                 { printf ("to at most %d\n", b->rhs); }
            fflush (stdout);
        }

        CC_MALLOC (c, 1, CCtsp_lpclique);
    ...
    if (*infeasible) {
        ...
        if (*infeasible) {
            if (!silent) {
                printf ("Problem is really infeasible (branch)\n");
                fflush (stdout);
            }
...
CLEANUP:
    if (rval == 0) {
        int sval = 0;
        void *tmp_ptr = (void *) lp->branchhistory;

        sval = CCutil_reallocrus_count (&tmp_ptr, 
                   lp->branchdepth + 1, sizeof (CCtsp_branchobj));
        if (sval) {
            fprintf (stderr, "CCutil_reallocrus_count failed\n"); return 1;
        }
        lp->branchhistory = (CCtsp_branchobj *) tmp_ptr;
...
int CCtsp_execute_unbranch (CCtsp_lp *lp, CClp_warmstart *warmstart,
        int silent, CCrandstate *rstate, int *infeasible)
{
    int rval = 0, j, n0, n1, num, depth = lp->branchdepth;
    CCtsp_branchobj *b;
    ...
    *infeasible = 0;
    ...
    if (lp->branchhistory[depth - 1].ends[0] != -1) {
        ...
        CCcheck_rval (rval, "find_branched_clique failed");
        if (!silent) {
    ...
    if (*infeasible) {
        ...
        if (*infeasible) {
            if (!silent) {
                printf ("Problem is really infeasible (unbranch)\n");
                fflush (stdout);
            }
            rval = CCtsp_update_result (lp);
            CCcheck_rval (rval, "CCtsp_update_result failed");
        ...
        int silent, CCrandstate *rstate)
{
    ...
    *b = (CCtsp_branchobj *) NULL;
    ...
    rval = CCtsp_bb_init_lp (&lp, probname, probnum, ncount, dat, ptour,
               *upperbound, pool, (CCtsp_lpcuts *) NULL, silent, rstate,
               &infeasible);
    CCcheck_rval (rval, "CCtsp_bb_init_lp failed");
    if (infeasible) {
        printf ("Initial BB LP is infeasible\n");  fflush (stdout);
        rval = 1; goto CLEANUP;
    }
        ...
        int *prune0, int *prune1, int silent, CCrandstate *rstate)
{
    ...
    *prune1 = 0;
    ...
    rval = CCtsp_bb_init_lp (&lp, probname, probnum, ncount, dat, ptour,
               initial_ub, pool, (CCtsp_lpcuts *) NULL, silent, rstate,
               &infeasible);
    CCcheck_rval (rval, "CCtsp_bb_init_lp failed");
    if (infeasible) {
        printf ("Initial BB LP infeasible\n");
        rval = 1; goto CLEANUP;
    }
        ...
        double *val, int *prune, int silent, CCrandstate *rstate)
{
    ...
    *prune = 0;
    ...
    rval = CCtsp_execute_branch (lp, b, silent, rstate, &infeasible);
    ...
    if (infeasible) {
        ...
        *val = newbound;
        ...
        if (lp->lowerbound >= lp->upperbound - 0.9) {
            ...
            CCcheck_rval (rval, "CCtsp_write_profile_id failed");
        ...
        int silent, CCrandstate *rstate)
{
    ...
    rval = CCtsp_execute_unbranch (lp, warmstart, silent, rstate, &infeasible);
    CCcheck_rval (rval, "CCtsp_execute_unbranch failed");
    if (infeasible) {
        fprintf (stderr, "unbranch created infeasible LP\n");
        rval = 1; goto CLEANUP;
    }
        ...
        int *tour, char *fname, int writeedges, int silent)
{
    ...
    if (!fname) sprintf (buf, "%s.sol", probname);
    else        sprintf (buf, "%s", fname);

    CC_MALLOC (cyc, ncount, int);
    for (i = 0; i < ncount; i++) cyc[i] = 0;
    for (i = 0; i < ncount; i++) cyc[tour[i]] = 1;
    for (i = 0; i < ncount; i++) {
        if (cyc[i] == 0) {
    ...
    for (i = 0; i < ncount; i++) cyc[i] = perm[tour[i]];
    ...
    if (dat->norm == CC_ROAD) {
        int j, a, b, tmp, total = 0;
        char mbuf[1024];
        FILE *missout = (FILE *) NULL;
        printf ("Checking tour for non-road edges ...\n"); fflush (stdout);

        if (!fname) sprintf (mbuf, "%s.miss", probname);
        else        sprintf (mbuf, "%s", fname);

        missout = fopen (mbuf, "w");
        if (missout == (FILE *) NULL) {
            perror (mbuf);
            fprintf (stderr, "Unable to open %s for output\n", mbuf);
            rval = 1; goto CLEANUP;
        }
        ...
        for (i = 0; i < ncount; i++) {
            a = tour[i]; b = tour[(i+1) % ncount];
            if (b < a) { CC_SWAP (a, b, tmp); }
