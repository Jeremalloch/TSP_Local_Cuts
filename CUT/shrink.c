...
int CCcut_SRK_identify_sets (CC_SRKgraph *G, int count, int *scount,
        int **slist) 
{
    int rval = 0;
    ...
    CC_SRKnode *n;
    int *hit = (int *) NULL;
    int *newcount = (int *) NULL;
    int ncount, i, j, k, t;

    nlist = CC_SAFE_MALLOC (count, CC_SRKnode **);
    CCcheck_NULL (nlist, "out of memory for nlist");
    for (i = 0; i < count; i++) nlist[i] = (CC_SRKnode **) NULL;

    for (i = 0; i < count; i++) {
        nlist[i] = CC_SAFE_MALLOC (scount[i], CC_SRKnode *);
        CCcheck_NULL (nlist[i], "out of memory for nlist[i]");
    }

    newcount = CC_SAFE_MALLOC (count, int);
    CCcheck_NULL (newcount, "out of memory for newcount");
    ...
    for (i = 0; i < ncount; i++) hit[i] = -1;

    for (i = 0; i < count; i++) {
        for (k = 0; k < scount[i]; k++) {
            t = slist[i][k];
            if (t < 0 || t >= ncount) {
                fprintf (stderr, "node %d out of scope\n", t);
                rval = 1;  goto CLEANUP;
            }
            if (hit[t] != -1) {
                fprintf (stderr, "duplicate node in lists\n");
                rval = 1;  goto CLEANUP;
            }
            hit[t] = i;
        }
    ...
    for (j = 0, n = G->head; n; n = n->next, j++) {
        if (hit[j] != -1) {
            nlist[hit[j]][newcount[hit[j]]] = n;
            newcount[hit[j]]++;
        }
    }

    for (i = 0; i < count; i++) {
        if (scount[i] != newcount[i]) {
    ...
    for (i = 0; i < count; i++) {
        for (k = 1; k < scount[i]; k++) {
            CCcut_SRK_identify_nodes (G, nlist[i][0], nlist[i][k]);
        }
    }
...
CLEANUP:
    ...
    if (nlist) {
        for (i = 0; i < count; i++) {
            CC_IFFREE (nlist[i], CC_SRKnode *);
        }
        CC_IFFREE (nlist, CC_SRKnode **);
    }
    CC_IFFREE (hit, int);
    CC_IFFREE (newcount, int);
    return rval;
        ...
        int **olist, double **olen, CC_SRKexpinfo *expand)
{
    ...
    *olen = (double *) NULL;
    ...
    if (ecount % 2) {
        fprintf (stderr, "Error A in grab_edges\n");
        rval = 1; goto CLEANUP;
    } else {
        ecount /= 2;
    }
    ...
    *olen  = CC_SAFE_MALLOC (ecount, double);
    ...
    if (k != ecount) {
        fprintf (stderr, "Error B in grab_edges\n");
        rval = 1; goto CLEANUP;
    }
