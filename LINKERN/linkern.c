        ...
        CCrandstate *rstate)
{
    ...
    while (myrnd < quitcount) {
        hit = 0;
        fstack.counter = 0;

        if (IMPROVE_SWITCH == -1 || myrnd < IMPROVE_SWITCH) {
...
#ifdef ACCEPT_BAD_TOURS
        if (myrnd % HEAT_RESET == HEAT_RESET - 1) {
...
printf ("%4d Steps   Best: %.0f   %.2f seconds (Negative %.0f) (%.0f)\n",
                          myrnd, t, CCutil_zeit () - szeit, t - best, oldbest);
                    fflush (stdout);
                }
                oldbest = best;
                best = t;
        ...
        myrnd++;
        if (silent == 0 && (hit || (myrnd % 1000 == 999))) {
            printf ("%4d Steps   Best: %.0f   %.2f seconds\n",
                               myrnd, best, CCutil_zeit () - szeit);
            fflush (stdout);
        }

        if (saveit_name && (myrnd % 10000 == 9999) && best < oldbest) {
