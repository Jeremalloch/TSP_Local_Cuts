...
/*    printf ("Slack %.6f\n", chunk_slack (c, &fault->a));*/
    ...
    CCutil_stop_timer (&timer->decompose, 1);
    if (rval) {
...
static void liberate_equality (CCchunk_graph *ch, CCchunk_ineq *a, CCchunk_lift_timer *timer)
{
    ...
    CCutil_stop_timer (&timer->liberate_equality, 1);
}
        ...
        paths_info *pinfo, CCchunk_lift_timer *timer)
{
  ...
  CLEANUP:
    CCutil_stop_timer (&timer->strengthen_equality, 1);
        ...
        paths_info *pinfo, CCchunk_lift_timer *timer)
{
  ...
  CLEANUP:
    CCutil_stop_timer (&timer->strengthen_edges, 1);
    CC_IFFREE (v.coef, int);
    return rval;
}
        ...
        paths_info *pinfo, CCchunk_lift_timer *timer)
{
  ...
  CLEANUP:
    CCutil_stop_timer (&timer->strengthen_work, 1);
    CC_IFFREE (z, int);
    CC_IFFREE (z1, int);
        ...
        int *yin, CCchunk_ineq *c, int *y, CCchunk_lift_timer *timer)
{
    ...
    for (;;) {
        ...
        CCutil_stop_timer (&timer->tilt_oracle, 1);
        ...
        if (counter >= MAX_TILT_ADJUST) {
            printf ("Hit maximum number of adjustments in tilt\n");
            rval = 1;
            goto CLEANUP;
        }
    }
    ...
    for (i=0; i<ecount; i++) {
        ...
        if (fixed[i] == 1) {
            ...
            rval = CCchunk_oracle (cnew, &anew, xsol, &v0, 0, MAXEFFORT,
                               &timer->oracle);
            CCutil_stop_timer (&timer->liberate_oracle, 1);
            if (rval) {
                if (rval == CC_CHUNK_ORACLE_SEARCHLIMITEXCEEDED) {
                    fprintf (stderr, "Hit ORACLE LIMIT in liberate_fixed\n");
                }
/*
...
*/
    ...
    for (i=0; i<ecount; i++) {
        ...
        if (fixed[i] == 0) {
            ...
            if (rval) {
                if (rval == CC_CHUNK_ORACLE_SEARCHLIMITEXCEEDED) {
                    fprintf (stderr, "Hit ORACLE LIMIT in liberate_fixed A\n");
                }
/*
                fprintf (stderr, "CCchunk_oracle failed in liberate_fixed\n");
*/
  ...
  CLEANUP:
    CCutil_stop_timer (&timer->liberate_fixed, 1);
    CC_IFFREE (anew.coef, int);
...
{
    ineq_info *s = (ineq_info *) u_data;
    CCtsp_lpcut_in *c = s->c;
...
static int lpcut_finish (int rhs, int *finished, void *u_data)
{
    ...
    rval = CCtsp_construct_skeleton (c, s->ncount);
    if (rval) {
