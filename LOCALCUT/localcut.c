...
/*      CCrandstate *rstate, double *fullzeit, double *maxviol)             */
/*   -fullzeit returns run time in seconds                                  */
/*   -maxviol is set to 0.0 (place holder for maximum violation)            */
/*                                                                          */
/****************************************************************************/
        ...
        double *fullzeit, double *maxviol)
{
    ...
    *cutcount = 0;
    ...
    if (flags.noshrink) {
        ...
        CCutil_stop_timer (&timer->find.shrink, 1);
        if (rval) {
    ...
    if (!silent)  printf ("\n");
#endif /* OLD_VERSION */
    st =  CCutil_stop_timer (&timer->all, 1);
    if (!silent) {
...
int CCchunk_localcut_single (CCtsp_lpcut_in **clist, int *cutcount, int ncount,
        ...
        int **pslist, int silent)
{
    ...
    if (!silent) {
        printf ("CCchunk_localcut_single ...\n"); fflush (stdout);
    }

    flags.uncivilized = 0;
    flags.noshrink = 0;
    flags.nolift = 0;
    flags.maxchunksize = pcount;

    outside = pslist[0][0];  /* Example of an outside node */
    ...
    *cutcount = 0;
    ...
    CCcut_SRK_identify_sets (&G, pcount, pscount, pslist);
    CCcheck_rval (rval, "CCcut_SRK_identify_sets failed");

    rval = CCcut_SRK_grab_edges (&G, &sncount, &secount, &selist, &sx,
                                 &cuts.expand);
    CCcheck_rval (rval, "CCcut_SRK_grab_edges failed");

    outhit = -1;
    for (i = 0; i < sncount && outhit == -1; i++) {
        for (j = cuts.expand.memindex[i]; j < cuts.expand.memindex[i+1]; j++) {
            if (cuts.expand.members[j] == outside) {
                outhit = i;
                break;
            }
        }
    ...
    if (outhit == -1) {
        printf ("Could not find the outside node\n");  fflush (stdout);
        rval = 1; goto CLEANUP;
    }

    rval = CCchunk_single (sncount, secount, selist, sx, outhit, &timer.find,
                           &ccb);
    CCcheck_rval (rval, "CCchunk_single failed");

    st =  CCutil_stop_timer (&timer.all, 1);
    if (!silent) {
        printf ("localcut_single done in %.2f seconds\n", st);
        fflush (stdout);
    }
        ...
        CCchunk_fault *fault, int *finished, void *u_data)
{
    ...
    *finished = 1;
    ...
    CCutil_stop_timer (&fcd->lift_t->all, 1);
    if (fcd->nolift >= 2) {
        printf ("faulty chunk\n");
