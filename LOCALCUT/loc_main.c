...

{
    ...
    int sncount, secount, k, j, cnt;
    int *slist = (int *) NULL;
    double *sx = (double *) NULL;
    int pcount, *pscount = (int *) NULL, **pslist = (int **) NULL; 
    ...
    CCcut_SRK_grab_edges (&G, &sncount, &secount, &slist, &sx, &expand);
    CCcheck_rval (rval, "CCcut_SRK_grab_edges failed");

    printf ("Shrunk Graph: %d nodes\n", sncount);

    if (sncount > 30) {
        printf ("Not going to test %d nodes\n", sncount);
        rval = 1; goto CLEANUP;
    }

    pcount = sncount;
    pscount = CC_SAFE_MALLOC (pcount, int);
    CCcheck_NULL (pscount, "out of memory for pscount");

    pslist = CC_SAFE_MALLOC (pcount, int *);
    CCcheck_NULL (pslist, "out of memory for pslist");
    for (i = 0; i < pcount; i++) pslist[i] = (int *) NULL;

    for (i = 0; i < pcount; i++) {
        cnt = expand.memindex[i+1] - expand.memindex[i];
        pscount[i] = cnt;
        pslist[i] = CC_SAFE_MALLOC (cnt, int);
        CCcheck_NULL (pslist[i], "out of memory for pslist[i]");
        for (j = 0; j < cnt; j++) {
            pslist[i][j] = expand.members[expand.memindex[i] + j];
        }
    }

    rval = CCchunk_localcut_single (&cuts, &cutcount, ncount, ecount,
               elist, x, pcount, pscount, pslist, run_silently);
    CCcheck_rval (rval, "CCchunk_localcut_single failed");
    ...
    CC_IFFREE (slist, int);
    CC_IFFREE (sx, double);
    CC_IFFREE (pscount, int);
    if (pslist) {
        for (i = 0; i < pcount; i++) {
            CC_IFFREE (pslist[i], int);
        }
        CC_IFFREE (pslist, int *);
    }
    ...
    rval = CCchunk_localcuts (&cuts, &cutcount, ncount, ecount, elist, x,
                 eps, flags, &timer, run_silently, &rstate, (double *) NULL,
                 (double *) NULL);
