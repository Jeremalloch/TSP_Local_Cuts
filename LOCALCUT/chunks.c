...
int CCchunk_single (int vcount, int ecount, int *elist, double *elen,
        ...
        CCchunk_chunk_callback *callback)
{
    ...
    rval = buildgraph (&G, vcount, ecount, elist, elen);
    CCcheck_rval (rval, "buildgraph failed");
    have_graph = 1;

    lastvertex = G.vlist + G.vcount;
    for (v = G.vlist; v != lastvertex; v++) v->name = OUT;

    chunkvertex = CC_SAFE_MALLOC (G.vcount, vertex *);
    CCcheck_NULL (chunkvertex, "out of memory for chunkvertex");
   
    for (j = 0, i = 1; i < G.vcount; i++, j++) {
        if (j == outside) j++;
        chunkvertex[i] = G.vlist + j;
        G.vlist[j].name = i;
    }

    c = grab_chunk (&G, chunkvertex, G.vcount);
    if (!c) {
        fprintf (stderr, "grab_chunk failed\n");
        rval = -1;  goto CLEANUP;
    }

    if (c->ncount > G.vcount) {
        fprintf (stderr, "chunk has no outside\n");
        rval = -1;  goto CLEANUP;
    }

    CCutil_start_timer (&timer->locate);
    rval = checkout_chunk (c, &timer->locate, callback);
    CCutil_stop_timer (&timer->locate, 1);
    if (rval) {
        printf ("checkout_chunk failed\n"); fflush (stdout);
        rval = 0;  goto CLEANUP;
    }

CLEANUP:

    if (have_graph) freegraph (&G);
    if (c) graph_chunk_old_free (c);
