...
int
    ...
    CCutil_edge_file_intersect (int ncount, char *fname1, char *fname2,
        int *ecount, int **elist, int **elen),
    CCutil_edge_file_difference (int ncount, char *fname1, char *fname2,
...
int
    ...
    CCcut_SRK_identify_pr_edges (CC_SRKgraph *G, double *minval, int *count,
        ...
        int *cutcount),
    CCcut_SRK_identify_sets (CC_SRKgraph *G, int count, int *scount,
        int **slist),
...
int
    CCedgegen_delaunay (int ncount, CCdatagroup *dat, int wantlist,
        int *ecount, int **elist, int *tricount, int **triangles);
...
int
    ...
    CCedgegen_xnear_build (int ncount, CCdatagroup *dat, double *wcoord,
        CCxnear *xn),
    CCedgegen_star_approx_k_nearest (int ncount, int num, CCdatagroup *dat,
...
int
    ...
    CClp_opt (CClp *lp, int method, int *infeasible),
...
#define CCtsp_PRICE_RCTHRESH  (-0.1)       /* to add a bad edge */
#define CCtsp_PRICE_MAXPENALTY (1000.0)    /* penalty permitted in addbad */
#else
...
#define CCtsp_VERIFY_PORT ((unsigned short) 24870)
#define CCtsp_DOMCUT_PORT ((unsigned short) 24871)
...
typedef struct CCtsp_lpcut {
    ...
    int                  *semimult;
    int                   coefcount;
    int                  *coef;         /* explicit coefficients for LHS */
    int                  *coefedges;    /* edges corresponding to coefs */
    int                   modcount;
    int                   age;
    int                   rhs;
    char                  sense;
    ...
    int                  *TP_handles;      /* 2 clique handles of TP cut */
    int                   TP_tricount;     /* #triominos */
    int                   TP_domcount0;    /* #dominos for handle 0 */
    int                   TP_domcount1;    /* #dominos for handle 1 */
    int                  *TP_dominos0;
    int                  *TP_dominos1;
    int                  *TP_tsets;
    int                  *TP_semicuts0;
...
typedef struct CCtsp_lpcut_in {
    int                   cliquecount;
    int                  *cliquemult;   /* clique multipliers if != NULL */
    int                   dominocount;
    int                   semicount;
    int                  *semimult;     /* semi-cut multipliers if != NULL */
    int                   coefcount;
    int                  *coef;         /* explicit coefficients for LHS */
    int                  *coefedges;    /* edges corresponding to coefs */
    int                   rhs;
    ...
    CCtsp_lpdomino       *semicuts;

    CCtsp_lpclique       *TP_handles;      /* 2 handles of TP cut   */
    int                   TP_tricount;     /* #triominos */
    int                   TP_domcount0;    /* #dominos for handle 0 */
    int                   TP_domcount1;    /* #dominos for handle 1 */
    CCtsp_lpdomino       *TP_dominos0;     /* length TP_domcount0 */
    CCtsp_lpdomino       *TP_dominos1;     /* length TP_domcount1 */
    CCtsp_lpclique       *TP_tsets;        /* length TP_2domcount */
    CCtsp_lpdomino       *TP_semicuts0;    /* length TP_2pdomcount */
    CCtsp_lpdomino       *TP_semicuts1;    /* length TP_2pdomcount */
...
typedef struct CCtsp_cutselect {
    ...
    int    exactblossom;
    int    consecutiveones;
    int    domino_tighten_lp;
    int    domino_tighten_pool;
    int    DP_cuts;
    int    TP_cuts;
    int    domino_safe_shrink;
    int    necklace;
    ...
    int    keep_cutting;   /* set to 1 to force root loop to keep cutting */
} CCtsp_cutselect;
...
int
    ...
    CCtsp_bfs_restart (char *probloc, char *restart_name, CCtsp_cutselect *sel,
        ...
        CCtsp_lpcuts *dominopool, int ncount,
    ...
    CCtsp_grunt (char *hostname, unsigned short hostport, char *poolfname,
        char *dompoolfname, char *cutbossname, char *domcutbossname,
        char *probloc, int silent, CCrandstate *rstate),
#endif /* CC_NETREADY */
    CCtsp_easy_dfs_brancher (CCtsp_lp *lp, CCtsp_cutselect *sel, int depth,
        double *upbound, int *bbcount, int usecliques, int *besttour,
        int longedge_branching, int simple_branching, 
        double *timebound, int *hit_timebound, int silent,
        CCrandstate *rstate),
...
int
    CCtsp_block_combs (CCtsp_lpcut_in **cuts, int *cutcount, int ncount,
        int ecount, int *elist, double *x, int silent, double *fullzeit,
        double *maxviol);
...
int
    ...
    CCtsp_execute_branch (CCtsp_lp *lp, CCtsp_branchobj *b,
        int silent, CCrandstate *rstate, int *infeasible),
...
int
    ...
    CCtsp_copy_lpcut_in_global (CCtsp_lpcut_in *c, CCtsp_lpcut_in *new,
    ...
    CCtsp_extract_dp_cut (CCtsp_lpcut_in *c, int *pndomino, int **pAcount,
...
int
    ...
    CCtsp_cutting_loop (CCtsp_lp *lp, CCtsp_cutselect *sel, int *snowtour,
        int savelp, int silent, CCrandstate *rstate, int *infeasible),
    CCtsp_subtour_loop (CCtsp_lp *lp, int silent, double ztol,
        CCrandstate *rstate, int *infeasible),
    CCtsp_subtour_loop_rc (CCtsp_lp *lp, int silent, double ztol,
        CCrandstate *rstate, int *infeasible),
    ...
    CCtsp_domino_loop (CCtsp_lp *lp, int silent, CCrandstate *rstate,
        int *infeasible),
    CCtsp_world_loop (CCtsp_lp *lp, int silent, int tmax, CCrandstate *rstate,
        int *infeasible),
    ...
    CCtsp_bb_cutting (char *probname, int probnum, int prob_newnum, int ncount,
        CCdatagroup *dat, int *ptour, double *upbound,
        CCtsp_lpcuts *pool, CCtsp_lpcuts *dominopool,
...
void
    ...
    CCtsp_cutselect_dominos (CCtsp_cutselect *s, int domsel, int safeshrink),
    CCtsp_cutselect_tighten (CCtsp_cutselect *s, int tighten),
    CCtsp_cutselect_xtighten (CCtsp_cutselect *s, int tighten),
    ...
    CCtsp_cutselect_remotedompool (CCtsp_cutselect *s, char *domcutbossname),
    CCtsp_cutselect_domboss (CCtsp_cutselect *s, char *dombossname),
    CCtsp_cutselect_keep_cutting (CCtsp_cutselect *s),
...
int
    ...
    CCtsp_teething_lp (CCtsp_lpcuts *cuts, CCtsp_tighten_info *stats,
        ...
        int *elist, double *x, double testtol, int maxcuts,
        double *viol, CCrandstate *rstate, double *fullzeit),
    CCtsp_comb2dp_lp (CCtsp_lpcuts *cuts, CCtsp_lpcut_in **cutsout,
    ...
    CCtsp_hypergraph_type (CCtsp_lpcut *c),
    CCtsp_hypergraph_type_in (CCtsp_lpcut_in *c),
    CCtsp_create_cut (int ncount, CCtsp_lpcut_in **cuts, int nsets,
        int *setsize, int **sets, int *setmultipliers, int nsemicuts,
        int **semisize, int ***semicuts, int *semimultipliers, int rhs),
    CCtsp_DP_cuts (CCtsp_lpcut_in **cuts, int *cutcount, int ncount,
        int ecount, int *elist, double *x, int safe_shrink,
        const char *bossname, double *fullzeit, double *maxviol),
    CCtsp_domino_tighten_lp (CCtsp_lpcuts *cuts, CCtsp_lpcut_in **cutsout,
        int *cutcount, int ncount, int ecount, int *elist, double *x,
        double testtol, int maxcuts, double *viol, CCrandstate *rstate,
        double *fullzeit),
    CCtsp_domino_tighten_pool (CCtsp_lpcuts *cuts, CCtsp_lpcut_in **cutsout,
        int *cutcount, int ncount, int ecount, int *elist, double *x,
        double testtol, int maxcuts, double *viol, CCrandstate *rstate,
        double *fullzeit),
    CCtsp_test_dp_cut (int ncount, CCtsp_lpcut_in *c, int *yes_no,
        int *large_domino),
    CCtsp_TP_cuts (CCtsp_lpcut_in **cuts, int *cutcount, int ncount,
        int ecount, int *elist, double *x, double *fullzeit, double *maxviol);
...
int CCtsp_geom_cuts (CCtsp_lpcut_in **cuts, int *cutcount, CCtsp_lp *tsplp,
...
int
    ...
    CCtsp_search_remotepool (char *remotehost, unsigned short remoteport,
        ...
        int ecount, int *elist, double *x, double *fullzeit),
    CCtsp_search_dominopool (CCtsp_lpcuts *pool, CCtsp_lpcut_in **cuts,
        int *cutcount, double *maxviol, int ncount, int ecount, int *elist,
...
int
    CCtsp_pr_cliquetree (CCtsp_lpcut_in **cuts, int *cutcount, int ncount,
        int ecount, int *elist, double *x, CCtsp_tighten_info *stats,
        double *fullzeit, double *outviol);
...
int
    ...
    CCtsp_load_lp (CCtsp_lp *lp, CCrandstate *rstate, int perturb, int silent),
    CCtsp_find_edge (CCtsp_lpgraph *g, int from, int to),
    CCtsp_inspect_full_edges (CCtsp_lp *lp),
    CCtsp_resparsify_lp (CCtsp_lp *lp, CCrandstate *rstate, int silent),
    ...
    CCtsp_get_lp_result (CCtsp_lp *lp, double *lb, double *ub, int *ecount,
        ...
        double **cut_pi),
    CCtsp_lpcut_in_nzlist (CCtsp_lpgraph *g, CCtsp_lpcut_in *c, int *pnzlist),
    ...
    CCtsp_addbad_variables (CCtsp_lp *lp, CCtsp_edgegenerator *eg,
        ...
        CCrandstate *rstate),
    CCtsp_eliminate_rebuild (CCtsp_lp *lp, int silent),
    ...
    CCtsp_reduced_cost_all (CCtsp_lp *lp, int *ecount, int **elist,
    ...
    CCtsp_dump_rc_all (CCtsp_lp *lp,  char *fname),
    CCtsp_dump_edge_all (CCtsp_lp *lp,  char *fname),
    CCtsp_dump_x (CCtsp_lp *lp, char *fname),
    CCtsp_read_x (int ncount, int *perm, char *fname, int *pxcount,
        int **pxlist, double **px),
    CCtsp_dump_shrunk_x (CCtsp_lp *lp, char *fname),
...
int
    ...
    CCtsp_subtour_dat (int ncount, CCdatagroup *indat, char *name,
        double *optval, int *subcount, int ***subtours, int *lpecount,
        int **lpelist, CClp_warmstart **lpwarm, int silent, double ztol,
        CCrandstate *rstate),
    CCtsp_call_linkern (int ncount, CCdatagroup *dat, int *tour,
...
int
    CCtsp_xtighten_lpcut_in (CCtsp_lpgraph *g, CCtsp_lpcut_in *c, double *x,
...
int
    CCpq_consecutiveones (CCtsp_lpcut_in **cuts, int *cutcount,
        ...
        double *x, double *fullzeit, double *maxviol);
...
int
    CCchunk_localcuts (CCtsp_lpcut_in **clist, int *cutcount, int ncount,
        ...
        double *fullzeit, double *maxviol),
    CCchunk_localcut_single (CCtsp_lpcut_in **clist, int *cutcount, int ncount,
        int ecount, int *elist, double *x, int pcount, int *pscount,
        int **pslist, int silent);
...
int
    ...
    CCchunk_single (int vcount, int ecount, int *elist, double *elen,
        int outside, CCchunk_find_timer *timer,
