...
/*    -fullzeit returns run time in second                                  */
...
CC_PTRWORLD_ROUTINES (intptrptr, intptrptr_alloc, intptrptr_bulk_alloc,
        intptrptr_free)
        ...
        neckgraph *g, CCrandstate *rstate)
{
    ...
    int i;
    int tried;
    intptrptr *found = (intptrptr *) NULL;
    int trynext;
    int status;
    int rval;
                                       ...
                                       CCtsp_lpcut_in **cuts, int *cutcount)
{
    ...
    rval = CCverify_cut (cut, ncount, CC_TYPE_COMB, (int *) NULL, 0,
                        (int *) NULL, (char *) NULL);
    if (rval == 0) {
        *cuts = cut;
        (*cutcount)++;
