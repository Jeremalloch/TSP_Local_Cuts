...
CC_PTRWORLD_ALLOC_ROUTINE (kpnode, kpnodealloc, kpnode_bulk_alloc)
...
static kpnode *build (int l, int u, double xlo, double xhi, double ylo,
        ...
        int bucketsize, int *cnt, CCrandstate *rstate, int silent);
        ...
        CCrandstate *rstate, int silent)
{
    ...
    *partlist = (int **) NULL;
    ...
    if ((norm & CC_NORM_SIZE_BITS) != CC_D2_NORM_SIZE &&
        (norm & CC_NORM_SIZE_BITS) != CC_D3_NORM_SIZE) {
    ...
    if (norm == CC_GEOM) {
        ...
        p->loson = build (0, target, xlo, pcutval, ylo, yhi, &thetree, dat->x,
                          dat->y, partsize, &scount, rstate, silent);
        if (!p->loson) {
        ...
        q->hison = build (m+1, ncount-1, qcutval, xhi, ylo, yhi, &thetree,
                          dat->x, dat->y, partsize, &scount, rstate, silent);
        if (!q->hison) {
    ...
    CC_MALLOC (*p_slist, scount, CCsubdiv);
    ...
    if (u - l + 1 <= bucketsize) {
        ...
        if (cutdim == 0) {
            ...
            p->loson = build (l, m, xlo, cutval, ylo, yhi, thetree, datx,
                              daty, bucketsize, cnt, rstate, silent);
            if (!p->loson) {
                rval = 1;  goto CLEANUP;
            }
        ...
        int **plist)
{
    ...
    if (n->loson) {
        ...
        k = *ind;
        CC_MALLOC (plist[k], cnt, int);
        for (i = 0; i < cnt; i++) { plist[k][i] = thetree->perm[n->lopt + i]; }
        slist[k].id = k;
        ...
        CCsubdiv *slist)
{
    ...
    CC_MALLOC (index_name, len, char);
    CC_MALLOC (new_name, len, char);
    CC_MALLOC (back_name, len, char);
        ...
        int *p_ncount, int *p_scount, CCsubdiv **p_slist)
{
    ...
    CC_MALLOC (slist, scount, CCsubdiv); 
    for (i = 0; i < scount; i++) {
        ...
        double *p_tourlen)
{
    ...
    CC_MALLOC (slist, scount, CCsubdiv_lkh); 
    for (i = 0; i < scount; i++) {
