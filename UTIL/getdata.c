...
/*                                                                          */
/*  int CCutil_matrix2dat (int ncount, int **M, CCdatagroup *dat)           */
/*    CONVERT a matrix of edge lengths to a CCdatagroup.                    */
/*     -M is an lower-triangular (or full) matrix of edge lengths; the      */
/*      values will be copied into dat.                                     */
/*                                                                          */
/*  int CCutil_tri2dat (int ncount, int *elen, CCdatagroup *dat)            */
/*    CONVERT an array-stored upper triangular matrix of edge lengths to a  */
/*      CCdatagroup                                                         */
/*     -elen contains the upper-triangular matrix (without the diagonal     */
/*      elements) as a single array; the values will be copied into dat.    */
/*                                                                          */
...
static int
    ...
    permute_road (int ncount, CCdatagroup *dat, int *perm),
...
int CCutil_datagroup_perm (int ncount, CCdatagroup *dat, int *perm)
{
    ...
    if (dat->norm == CC_USER) {
        rval = permute_user (ncount, &dat->userdat, perm);
        CCcheck_rval (rval, "permute_user failed");
        goto CLEANUP;
    } else if (dat->norm == CC_RHMAP1 || dat->norm == CC_RHMAP2 ||
               dat->norm == CC_RHMAP3 || dat->norm == CC_RHMAP4 ||
               dat->norm == CC_RHMAP5) {
        rval = permute_rhdata (ncount, &dat->rhdat, perm);
        CCcheck_rval (rval, "permute_rhdata failed");
        goto CLEANUP;
    } else if (dat->norm == CC_SPARSE) {
        rval = permute_sparse (ncount, dat, perm);
        CCcheck_rval (rval, "permute_sparse failed");
        goto CLEANUP;
    } else if (dat->norm == CC_ROAD) {
        rval = permute_road (ncount, dat, perm);
        CCcheck_rval (rval, "permute_road failed");
        goto CLEANUP;
    }
    ...
    if (dat->x != (double *) NULL) {
        ...
        for (i = 0; i < ncount; i++) { tempx[i] = dat->x[perm[i]]; }
    ...
    if (dat->y != (double *) NULL) {
        ...
        for (i = 0; i < ncount; i++) { tempy[i] = dat->y[perm[i]]; }
    ...
    if (dat->z != (double *) NULL) {
        ...
        for (i = 0; i < ncount; i++) { tempz[i] = dat->z[perm[i]]; }
...
void CCutil_cycle_len (int ncount, CCdatagroup *dat, int *cycle, double *len)
{
    ...
    *len = 0.0;
    for (i = 1; i < ncount; i++) {
        ...
        if (CCutil_dat_edgelen (cycle[i-1], cycle[i], dat) > 80467) {
            printf ("%d %d %d\n", cycle[i-1], cycle[i], CCutil_dat_edgelen (cycle[i-1], cycle[i], dat));
        }
    } 
...
int CCutil_matrix2dat (int ncount, int **M, CCdatagroup *dat)
{
    ...
    CCutil_init_datagroup (dat);
    dat->adj = CC_SAFE_MALLOC (ncount, int *);
    CCcheck_NULL (dat->adj, "out of memory for dat->adj");
    dat->adjspace = CC_SAFE_MALLOC (ncount * (ncount+1) / 2, int);
    CCcheck_NULL (dat->adjspace, "out of memory for dat->adjspace");
    ...
    rval = CCutil_dat_setnorm (dat, CC_MATRIXNORM);
    CCcheck_rval (rval, "CCutil_dat_setnorm failed");
...
int CCutil_tri2dat (int ncount, int *elen, CCdatagroup *dat)
{
    ...
    if (dat->adj == (int **) NULL ||
        dat->adjspace == (int *) NULL) {
        fprintf (stderr, "Out of memory in CCutil_tri2dat\n");
        rval = 1;
        goto CLEANUP;
    }
        ...
        int defaultlen, CCdatagroup *dat)
{
    ...
    if (dat->adj == (int **) NULL ||
        dat->adjspace == (int *) NULL) {
        fprintf (stderr, "Out of memory in CCutil_graph2dat\n");
        rval = 1;
        goto CLEANUP;
    }
    ...
    int *in_elist, int *in_elen, int *ecount, int **elist, int **elen)
{
    ...
    *elen  = (int *) NULL;
    ...
    CCutil_dat_getnorm (dat, &norm);
    if (norm != CC_SPARSE) {
        fprintf (stderr, "CCutil_sparse_strip_edges called with a norm\n");
        rval = 1; goto CLEANUP;
    }
...
{
    int rval = 0, i, j, k, ecount = dat->sparse_ecount;
    int **tadj     = (int **) NULL, *tadjspace = (int *) NULL;
    int **tlen     = (int **) NULL, *tlenspace = (int *) NULL;
    int *tdegree   = (int *) NULL,  *inv       = (int *) NULL;

    CC_MALLOC (tadjspace, ecount, int);
    CC_MALLOC (tlenspace, ecount, int);
    CC_MALLOC (tadj, ncount, int *);
    CC_MALLOC (tlen, ncount, int *);
    CC_MALLOC (tdegree, ncount, int);
...
static int permute_road (int ncount, CCdatagroup *dat, int *perm)
{
    int rval = 0, i;
    double *tempx = (double *) NULL, *tempy = (double *) NULL;

    if (!dat->x || !dat->y || !dat->adj) {
