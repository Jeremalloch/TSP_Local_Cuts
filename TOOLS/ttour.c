...
/*                                                                          */
/*  Written by:  Cook                                                       */
/*  Date: April 16, 2003; October 4, 2010.                                  */
/*                                                                          */
/*                                                                          */
/****************************************************************************/
...
static int
    ...
    cat_full_tour (int ncount, CCdatagroup *dat, char *name, int pcount,
        CCsubdiv_lkh *trac, double origval, int use_tsplib),
    ...
    get_part_data (char *name, int ind, int *p_count, int **p_namelist,
        int **p_tour, char *tname, int use_default, int use_tsplib),
...
static int cat_full_tour (int ncount, CCdatagroup *dat, char *name, int pcount,
        CCsubdiv_lkh *lkhtrac, double origval, int use_tsplib)
{
    int rval = 0;
    int i, j, k = 0;
    int *tour = (int *) NULL;
...
{
    int i, rval = 0;
    int n0 = 0, n1 = 0, m0 = 0, m1 = 0, bcount = 0;
    int *btour = (int *) NULL;
    int *bnamelist = (int *) NULL;
    char *hits = (char *) NULL;
    char buf[1024];
    char tnam[128];
    double bval, sumval = 0.0;
    ...
    rval = get_part_data (name, ptour[0], &bcount, &bnamelist, &btour, tnam,
                          0, 0);
    CCcheck_rval (rval, "get_part_data failed");
    for (i = 0; i < bcount; i++) {
        ...
        int **p_btour)
{
    ...
    rval = get_part_data (name, ip, &pcount, &pnamelist, &ptour, tnam, 0, 0);
    CCcheck_rval (rval, "get_part_data failed");

    rval = get_part_data (name, iq, &qcount, &qnamelist, &qtour, tnam, 0, 0);
    CCcheck_rval (rval, "get_part_data failed");
        ...
        CCdatagroup *dat)
{
    ...
    for (i = 0; i < pcount; i++) {
        rval = get_part_data (name, i, &count, &namelist, &tour, tnam, 0, 0);
        CCcheck_rval (rval, "get_part_data failed");
        for (j = 1; j < count; j++) {
        ...
        int **p_tour, char *tname, int use_default, int use_tsplib)
{
    ...
    if (p_tour) {
        if (use_tsplib) {
            sprintf (tourbuf, "%s_tour.%d", name, ind);
        } else {
    ...
    if (p_tour) {
        ...
        if (use_tsplib) {
            rval = CCutil_getcycle_tsplib (count, tourbuf, tour);
        } else {
...
static int parseargs (int ac, char **av)
{
    ...
    while ((c = CCutil_bix_getopt (ac, av, "bcBD:I:P:rs:tT:qwN:?", &boptind, &boptarg)) != EOF) { 
        switch (c) {
        case 's':
            seed = atoi(boptarg);
            break;
...
static void usage (char *fname)
{
    ...
    fprintf (stderr, "   -t    subpaths are in tsplib format (for cat)\n");
