...
static int
    draw_tree (char *probname, double restart_upbound, int maxdepth,
...
static void
    ...
    draw_postlude (void),
    draw_compute_t (tsp_bbnode *bbnode, double lobound, double upbound),
    ...
    draw_subtree_nodes (tsp_bbnode *bbnode, int depth, int maxdepth,
        double upbound),
    draw_compute_t_subtree (tsp_bbnode *bbnode, int depth, int maxdepth,
...
int main (int ac, char **av)
{
    ...
    if (use_graphics) {
        rval = draw_tree (probname, restart_upbound, arg_maxdepth, rootbbnode);
        if (rval) {
...
static int draw_tree (char *probname, double restart_upbound,
        int maxdepth, tsp_bbnode *rootbbnode)
{
    int drawcnt = 0;
    ...
    draw_compute_t_subtree (rootbbnode, 0, maxdepth, rootbbnode->lowerbound,
        restart_upbound);
        ...
        char *probname)
{
    ...
    printf ("newpath 0 %.2f loc moveto 0 %.2f loc lineto stroke\n",
            (double) (DRAWING_HEIGHT + DRAWING_BOTTOM), 
            (double) DRAWING_BOTTOM);
    
    for (m = labello; m <= upbound; m += labelstep) {
        t = (((upbound - m) * ((double) DRAWING_HEIGHT)) /
            (upbound - lobound)) +
            ((double) DRAWING_BOTTOM);
        printf ("(%.0f) 0 %.2f lbl\n", m, t);
    }
...
{
    printf ("%d %.2f %d ", bbnode->number, bbnode->t, depth);
...
static void draw_compute_t (tsp_bbnode *bbnode, double lobound, double upbound)
{
    double t;

    t = (((upbound - bbnode->lowerbound) * ((double) DRAWING_HEIGHT)) /
        (upbound - lobound)) +
        ((double) DRAWING_BOTTOM);
...
static void draw_compute_t_subtree (tsp_bbnode *bbnode, int depth, int maxdepth,
        double lobound, double upbound)
{
    draw_compute_t (bbnode, lobound, upbound);
    if (bbnode->child0 != (tsp_bbnode *) NULL &&
        (maxdepth <= 0 || depth < maxdepth) &&
        bbnode->child0->lowerbound <= upbound - 1.0) {
        draw_compute_t_subtree (bbnode->child0, depth+1, maxdepth, lobound,
                                 upbound);
    }
    if (bbnode->child1 != (tsp_bbnode *) NULL &&
        (maxdepth <= 0 || depth < maxdepth) &&
        bbnode->child1->lowerbound <= upbound - 1.0) {
        draw_compute_t_subtree (bbnode->child1, depth+1, maxdepth, lobound,
                                upbound);
    }
}
...
#define NAMESTRNUM(xa,xb,xc) (((xa)-'a') * 1296 + ((xb)-'a'+10) * 36 + \
    ...
    } else if (n < 1000 + (26*36*36 - 5) + 36*36*36*36 + 36*36*36*36*36) {
        /* US50k: Extend names to 5 digits for big search tree */
        n -= 1000;
        n -= 26*36*36 - 5;
        n -= 36*36*36*36;
...
#if 0
/* OLD version, before US50K increase to 5 characters */
static int prob_name (char *buf, size_t buflen, const char *f, int n)
{
    int l = (int) strlen(f);
    int lastslash;
