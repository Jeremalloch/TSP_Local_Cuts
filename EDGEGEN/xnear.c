...
/* The GEOM norm is not supported by concorde's kd-tree code, so the     */
/* quad-nearest computations are slow for large instances (10mil pints). */
/* Since the Karp partition just subdivides the points into regions, the */
/* EUCLIDEAN quad-nearest should give a reasonable approximation.  This  */
/* computes the quad-nearest with EUCLIDEAN but outputs the edges with   */
/* with GEOM.  It takes a datfile as input (not a TSPLIB file).          */

/* Since we work with int lengths, the EUCLIDEAN data will be scaled by  */
/* 1000000 to get precision comparable to GEOM.                          */

int CCedgegen_star_approx_k_nearest (int ncount, int num, CCdatagroup *dat,
        int *ecount, int **elist, int silent)
{
    ...
    CCutil_dat_getnorm (dat, &norm);
    if (norm != CC_GEOM) {
        fprintf (stderr, "Must have GEOM norm in star problems\n");
        rval = 1; goto CLEANUP;
    }

    rval = CCutil_copy_datagroup (ncount, dat, &sdat);
    CCcheck_rval (rval, "CCutil_copy_datagroup failed");

    CCutil_dat_setnorm (&sdat, CC_EUCLIDEAN);
    for (i = 0; i < ncount; i++) {
        sdat.x[i] *= 1000000.0;
        sdat.y[i] *= 1000000.0;
    }

    rval = CCedgegen_x_k_nearest (ncount, num, &sdat, (double *) NULL, 1,
                                  ecount, elist, silent);
    CCcheck_rval (rval, "CCedgegen_x_k_nearest failed");
