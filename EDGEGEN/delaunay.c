...
/*  Date: March 12, 1996                                                    */
/*        March 8, 2012 (bico), free all memory allocated by vor_myalloc    */
/*                                                                          */
/*                                                                          */
/*    EXPORTED FUNCTIONS:                                                   */
/*                                                                          */
...
/*      -tricount returns number of triangles if not NULL                   */
/*      -triangles returns trianges as triples if not NULL                  */
/*                                                                          */
/****************************************************************************/
...
typedef struct pcharptr {
    char *this;
    struct pcharptr *next;
} pcharptr;
...
CC_PTRWORLD_LIST_ROUTINES (pcharptr, char *, pcharptralloc, pcharptr_bulk_alloc,
        pcharptrfree, pcharptr_listadd, pcharptr_listfree)
CC_PTRWORLD_LEAKS_ROUTINE (pcharptr, pcharptr_check_leaks, this, char *)
...
static int
    out_triple (delaunaydat *dd, struct Site *s1, struct Site *s2,
        struct Site *s3, int *ntotal, int wantlist),
        ...
        int *ecount, int **elist, int *tricount, int **triangles)
{
    ...
    CCptrworld_init (&dd.pcharptr_world);
    dd.total_alloc = 0;
    dd.vormemory = (pcharptr *) NULL;
    dd.table = (intptr **) NULL;
    dd.triangles = (int *) NULL;
    dd.tricount = 0;
    ...
    if (triangles) *triangles = (int *) NULL;

    /* at most 2*ncount triangles, so allocated 6*ncount ints */
    ...
    if (triangles) {
        *triangles = CC_SAFE_MALLOC (6*ncount, int);
        dd.triangles = *triangles;
    }

    dd.table = CC_SAFE_MALLOC (ncount, intptr *);
    CCcheck_NULL (dd.table, "out of memory for dd.table");
    for (i = 0; i < ncount; i++) {
    ...
    while (1) {
        ...
        if (newsite != (struct Site *) NULL
            ...
            rval = out_triple (&dd, bot, top, rightreg (&dd, lbnd), &ntotal,
                               wantlist);
            CCcheck_rval (rval, "out_triple failed");
    ...
    if (tricount) *tricount = dd.tricount;
    ...
    /* Free all memory allocated by vor_myalloc */
    for (pp = dd.vormemory; pp; pp = pp->next) {
        free (pp->this);
    }
    pcharptr_listfree (&dd.pcharptr_world, dd.vormemory);

    if (pcharptr_check_leaks (&dd.pcharptr_world, &total, &onlist)) {
        fprintf (stderr, "WARNING: %d outstanding pcharptrs in delaunay\n",
                 total - onlist);
    }
...
CLEANUP:
    ...
    CCptrworld_delete (&dd.pcharptr_world);
    CC_IFFREE (dd.table, intptr *);
    CC_IFFREE (dd.sites, struct Site);
        ...
        struct Site *s3, int *ntotal, int wantlist)
{
    ...
    if (dd->triangles) {
        dd->triangles[3*dd->tricount]   = s1->sitenbr;
        dd->triangles[3*dd->tricount+1] = s2->sitenbr;
        dd->triangles[3*dd->tricount+2] = s3->sitenbr;
        dd->tricount++;
    }
...
static char *vor_myalloc (delaunaydat *dd, unsigned n)
{
    ...
    if (pcharptr_listadd (&dd->vormemory, t, &dd->pcharptr_world)) {
        fprintf (stderr, "Out of memory for pcharptr\n");
        exit (1);
    }
