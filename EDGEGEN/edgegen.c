...
CC_PTRWORLD_ROUTINES (intptr, intptralloc, intptr_bulk_alloc,
        intptrfree)
CC_PTRWORLD_LISTADD_ROUTINE (intptr, int, intptr_listadd, intptralloc) 
        ...
        tabledat *td, int silent, CCrandstate *rstate)
{
    ...
    for (myrnd = 0; myrnd < number; myrnd++) {
        k = td->tabletotal;
        tzeit = CCutil_zeit ();
        ...
        if (!silent) {
            printf ("  Random tour %d: %.0f, added %d edges (%.2f seconds)\n",
                     myrnd, val, td->tabletotal - k, CCutil_zeit () - tzeit);
            fflush (stdout);
        }
    }
        ...
        tabledat *td, int silent, CCrandstate *rstate)
{
    ...
    CCutil_dat_getnorm (dat, &norm);
    if ((norm & CC_NORM_BITS) == CC_KD_NORM_TYPE) {
        if (CCkdtree_qboruvka_tour (kt, ncount, dat, tour, &val, silent,
                                    rstate)) {
        ...
        int silent, CCrandstate *rstate)
{
    ...
    int k, myrnd, current = td->tabletotal;
            ...
            if (!silent) {
                if (use_3opt)
                    printf ("  3OPT tour %d (from %.0f): %.0f, added %d edges (%.2f sec)\n",
                            myrnd, ival, val, td->tabletotal - k,
                            CCutil_zeit () - tzeit);
                else if (two_and_a_half)
        ...
        CCedgegengroup *plan, tabledat *td, int silent, CCrandstate *rstate)
{
    ...
    if ((norm & CC_NORM_BITS) == CC_KD_NORM_TYPE) {
        if (plan->linkern.greedy_start) {
            ...
            tzeit = CCutil_zeit ();
            rval = CCkdtree_qboruvka_tour (kt, ncount, dat, gtour, &gval, 
                                           silent, rstate);
            CCcheck_rval (rval, "CCkdtree_qboruvka_tour failed");
            if (!silent) {
    ...
    for (myrnd = 0; myrnd < plan->linkern.count; myrnd++) {
        tzeit = CCutil_zeit ();
        k = td->tabletotal;
        if (gtour != (int *) NULL) {
            for (i = 0; i < ncount; i++)
        ...
        if (!silent) {
            printf ("  LK tour %d (from %.0f): %.0f, added %d edges (%.2f sec)\n",
                 myrnd, ival, val, td->tabletotal - k, CCutil_zeit () - tzeit);
            fflush (stdout);
        }
    }
        ...
        int iterations, tabledat *td, int silent, CCrandstate *rstate)
{
    ...
    if (CCedgegen_mlinkern (ncount, dat, 1, &tcount, &tlist, kt, iterations-1,
        rstate)) {
