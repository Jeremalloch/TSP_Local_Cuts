...
/****************************************************************************/
/*  Modified version of BIGGUY/bigguy.c                                     */
/*                                                                          */
/*  BIGGUY OPERATIONS:                                                      */
/*                                                                          */
/*  x += y                  BBbigguy_add (BBbigguy *x, BBbigguy y)          */
/*  x -= y                  BBbigguy_sub (BBbigguy *x, BBbigguy y)          */
/*  x += y*m                BBbigguy_addmult (BBbigguy *x, BBbigguy y,      */
/*                                            int m)                        */
/*  x<y -1, x==y 0, x>y 1   BBbigguy_cmp (BBbigguy x, BBbigguy y)           */
/*  ceil(x)                 BBbigguy_ceil (BBbigguy x)                      */
/*                                                                          */
/*  (double) x              BBbigguy_bigguytod (BBbigguy x)                 */
/*  (bigguy) i              BBbigguy_itobigguy (int i)                      */
/*  (bigguy) d              BBbigguy_dtobigguy (double d)                   */
/*                                                                          */
/*  read(x)                 BBbigguy_sread (BB_SFILE *f, BBbigguy *x)       */
/*  write(x)                BBbigguy_swrite (BB_SFILE *f, BBbigguy x)       */
/*                                                                          */
/*  biggest possible x      const BBbigguy BBbigguy_MAXBIGGUY               */
/*  smallest possible x     const BBbigguy BBbigguy_MINBIGGUY               */
/*  0                       const BBbigguy BBbigguy_ZERO                    */
/*  1                       const BBbigguy BBbigguy_ONE                     */
/*                                                                          */
/*                                                                          */
/*  EXPORTED FUNCTIONS:                                                     */
/*                                                                          */
/*  int BBbigguy_swrite (BB_SFILE *f, BBbigguy x)                           */
/*                                                                          */
/*  int BBbigguy_sread (BB_SFILE *f, BBbigguy *x)                           */
/*                                                                          */
/*  void BBbigguy_addmult (BBbigguy *x, BBbigguy y, int m)                  */
/*    If an overflow occurs, routine writes error message and aborts        */
/*                                                                          */
/*  int BBbigguy_cmp (BBbigguy x, BBbigguy y)                               */
/*                                                                          */
/*  double BBbigguytod (BBbigguy x)                                         */
/*                                                                          */
/*  BBbigguy BBbigguy_itobigguy (int d)                                     */
/*                                                                          */
/*  BBbigguy BBbigguy_dtobigguy (double d)                                  */
/*    If an overflow occurs, routine writes error message and aborts        */
/*                                                                          */
/*  BBbigguy BBbigguy_ceil (BBbigguy x)                                     */
/*    If an overflow occurs, routine writes error message and aborts        */
/*                                                                          */
/****************************************************************************/
...
const BBbigguy BBbigguy_MINBIGGUY = {0x8000,0x0000,0x0000,0x0001};
const BBbigguy BBbigguy_MAXBIGGUY = {0x7fff,0xffff,0xffff,0xffff};
const BBbigguy BBbigguy_ZERO = {0,0,0,0};
const BBbigguy BBbigguy_ONE  = {0,1,0,0};


static void
    bigguy_neg (BBbigguy *x);


static void bigguy_neg (BBbigguy *x)
{
...
int BBbigguy_swrite (BB_SFILE *f, BBbigguy x)
{
    if (BBsafe_swrite_ushort (f, x.ihi)) return -1;
    if (BBsafe_swrite_ushort (f, x.ilo)) return -1;
    if (BBsafe_swrite_ushort (f, x.fhi)) return -1;
    if (BBsafe_swrite_ushort (f, x.flo)) return -1;
...
int BBbigguy_sread (BB_SFILE *f, BBbigguy *x)
{
    if (BBsafe_sread_ushort (f, &(x->ihi))) return -1;
    if (BBsafe_sread_ushort (f, &(x->ilo))) return -1;
    if (BBsafe_sread_ushort (f, &(x->fhi))) return -1;
    if (BBsafe_sread_ushort (f, &(x->flo))) return -1;
    return 0;
}

double BBbigguy_bigguytod (BBbigguy x)
{
    int sgn = 1;
...
BBbigguy BBbigguy_itobigguy (int d)
{
    ...
    if (d < 0 || (d >> 16) >= 32768) {
        fprintf (stderr, "OVERFLOW in BBbigguy_itobigguy\n");
        fprintf (stderr, "BIGGUY errors are fatal\n");
        abort ();
    }
...
BBbigguy BBbigguy_dtobigguy (double d)
{
    ...
    if (d / 65536.0 >= 32768.0) {
        fprintf (stderr, "OVERFLOW in BBbigguy_dtobigguy (%.6f)\n", d * sgn);
        fprintf (stderr, "BIGGUY errors are fatal\n");
        abort ();
    }
...
void BBbigguy_addmult (BBbigguy *x, BBbigguy y, int m)
{
    ...
    if (m == -m && m != 0) {
        fprintf (stderr, "OVERFLOW in BBbigguy_addmult (1)\n");
        fprintf (stderr, "BIGGUY errors are fatal\n");
        abort ();
    }
    ...
    if (mlo < -32768 || mlo > 32767) {
        fprintf (stderr, "OVERFLOW in BBbigguy_addmult (2)\n");
        fprintf (stderr, "BIGGUY errors are fatal\n");
        abort ();
    }
    if (mhi < -32768 || mhi > 32767) {
        fprintf (stderr, "OVERFLOW in BBbigguy_addmult (3)\n");
        fprintf (stderr, "BIGGUY errors are fatal\n");
        abort ();
    }
    ...
    if (carry < -1 || carry > 1 ||
        ...
        (carry == 0 && oldsgn != sgn) ||
        (carry == 1 && !(oldsgn == -1 && sgn == 1))) {
        fprintf (stderr, "OVERFLOW in BBbigguy_addmult (4)\n");
        fprintf (stderr, "BIGGUY errors are fatal\n");
        abort ();
    }
