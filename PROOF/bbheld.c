...
/****************************************************************************/
/*                                                                          */
/*  Held-Karp for small TSP instances (from heldkarp.c)                     */
/*                                                                          */
...
/*  EXPORTED FUNCTIONS:                                                     */
/*                                                                          */
/*  int BBheld_small_elist (int ncount, int ecount, int *elist,             */
...
#define BB_INT_MAX ((int) (~(((unsigned) 1) << ((8*sizeof(int))-1))))
...
int BBheld_small_elist (int ncount, int ecount, int *elist, int *elen,
        ...
        int *tour_elist, int nodelimit, int silent)
{
    ...
    *foundtour = 0;
    ...
    if (ncount > MAX_NODES) {
        ...
        rval = BB_HELDKARP_ERROR; goto CLEANUP;
    }
    for (i = 0; i < ecount; i++) {
    ...
    adjlist  = BB_SAFE_MALLOC (ncount-1, int *);
    padjlist = BB_SAFE_MALLOC ((ncount-1)*(ncount-1), int);
    zadjlist = BB_SAFE_MALLOC (ncount, int);
    len      = BB_SAFE_MALLOC (ecount, int);
    ...
    tree = BB_SAFE_MALLOC (ncount, int);
    y    = BB_SAFE_MALLOC (ncount, int);
    deg  = BB_SAFE_MALLOC (ncount, int);
    besttour = BB_SAFE_MALLOC (ncount, int);
    ...
    efix   = BB_SAFE_MALLOC (ecount, int);
    degfix = BB_SAFE_MALLOC (ncount, int);
    ...
    if (nodelimit != -1 && bbcount > nodelimit) {
        rval = BB_HELDKARP_SEARCHLIMITEXCEEDED;
    } else {
        *optval = (double) val;
    }
...
CLEANUP:
    ...
    BB_IFFREE (adjlist, int *);
    BB_IFFREE (padjlist, int);
    BB_IFFREE (zadjlist, int);
    BB_IFFREE (degfix, int);
    BB_IFFREE (tree, int);
    BB_IFFREE (efix, int);
    BB_IFFREE (len, int);
    BB_IFFREE (deg, int);
    BB_IFFREE (y, int);
    BB_IFFREE (besttour, int);
    return rval;
}
...
static void initial_y (int ncount, int ecount, int *elist, int *len, int *y)
{
    ...
    for (i = 0; i < ncount; i++) y[i] = BB_INT_MAX;
    for (i=0; i < ecount; i++) {
    ...
    *notree = 0;
    ...
    for (i = 1; i < ncount; i++) {
        ...
        if (e > 0) {
            ...
            if (min2 == -BB_INT_MAX) { *notree = 1; return; }
            min2 = min1;
            min1 = -BB_INT_MAX;
            emin2 = emin1;
            emin1 = (-e)-1;
...
{
    int nbnd, nadd, nrem, cur, minnode, i, we, e, ycur, minlen;
    int nremain[MAX_NODES], nedge[MAX_NODES], nlen[MAX_NODES];
    int *tptr;
...
#if 0   /* Alternative branching rule */
    for (i = 0; i < ncount; i++) {
        e = tree[i];
